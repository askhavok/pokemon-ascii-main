...
printPokemonStats(party[currentpokemon], maxhp[currentpokemon], hp[currentpokemon]) # Display the stats of the user's current pokemon
...
while (hp.count(0) != len(hp)) and (enemyhp > 0):
    ...
    if battledecisions[decision] == 'Attack':
        ...
        if attacks[currentpokemon][attack] != 'Go Back':
            ...
            printTextBox(str(party[currentpokemon]) + ' used ' + str(attacks[currentpokemon][attack]) + '! ' + str(enemyname) + ' took ' + str(movedmg[currentpokemon][attack]) + ' damage!') # Display the user's attack, and the damage it dealt
            ...
            printTextBox(str(enemyname) + ' used ' + str(enemyattacks[attack]) + '! ' + str(party[currentpokemon]) + ' took ' + str(enemymovedmg[attack]) + ' damage!') # Display the enemy's attack, and the damage it dealt
            ...
            if inventory[item] != 'Go Back':
                if inventory[item] == 'Elixir':
                    ...
                    if attacks[currentpokemon][attack] != 'Go Back':
                        ...
                        printTextBox('You used ' + str(item) + '! ' + str(party[currentpokemon]) + ' gained 10 PP!') # Display the used item, and how much PP their pokemon gained
                        del(inventory[item])
                    ...
                    printTextBox('You used ' + str(item) + '! ' + str(party[currentpokemon]) + ' gained 10 HP!') # Display the used item, and how much HP their pokemon gained
                    del(inventory[item])
        ...
        party.append('Go Back') # Add 'Go Back' to the list of options that the user can choose
        printOptionList(party) # Display the names of pokemon that the user can choose

        while True:
            newpokemon = getUserDecision('Which pokemon do you want to summon?', party) # Get the user's input for the new pokemon they want to switch to

            if party[newpokemon] == 'Go Back':
                break # Exit the loop
            elif newpokemon == currentpokemon:
                printTextBox(str(party[currentpokemon]) + ' is already summoned!') # If the pokemon they want to switch to is the same as the current pokemon, then tell the user that it's already summoned
            else:
                break # Exit the loop

        if party[newpokemon] != 'Go Back':
            printTextBox('You summoned ' + str(party[newpokemon]) + '!') # Display the pokemon that the user summoned

            printPokemonStats(enemyname, enemymaxhp, enemyhp) # Display the stats of the enemy's pokemon
            printPokemonStats(party[newpokemon], maxhp[newpokemon], hp[newpokemon]) # Display the stats of the user's new pokemon

            currentpokemon = newpokemon # Set the new pokemon as the user's current pokemon

        party.remove('Go Back')
